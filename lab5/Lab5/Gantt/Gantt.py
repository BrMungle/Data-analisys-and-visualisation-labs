...
schedule['Event plot'] = list(map(lambda x: help_func_2(x,35),list(schedule['Event'])))
...
x_labels = [x.strftime('%d - %m - %Y') for x in x_labels]
...
for i in range(len(schedule['Begin'])):
    ax.broken_barh([(schedule['Begin'][i], schedule['Width'][i])], 
                    (schedule['Event id'][i] - 0.4, 0.8), facecolors =('black')) 
...
plt.yticks(size =10)
fig.subplots_adjust(bottom=0.1, top = 0.99, left = 0.4, right = 0.99)
fig.savefig('plots/BW_Gantt_chart.png')